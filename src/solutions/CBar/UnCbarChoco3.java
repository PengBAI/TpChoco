...
public class UnCbarChoco3 {
	...
	public static void main(String[] args) {
		...
		LecteurFichier monlecteurdefichier = new LecteurFichier(
				"src/solutions/CBar/dataUnCbar.txt");
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				if (i != j) {
					UnCbarSolv.post(LCF
							.or(ICF.arithm(C[j], "-", C[i], ">=", p[j]).reif(),
									ICF.arithm(C[i], "-", C[j], ">=", p[i])
											.reif()));
				}
		...
		System.out.println(UnCbarSolv.getMeasures().getSolutionCount()
